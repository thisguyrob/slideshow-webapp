#!/usr/bin/env bash
# ------------------------------------------------------------
# scavenger_hunt_slideshow_builder_corrected.sh - Properly Fixed
# ------------------------------------------------------------
...
echo "🎬 Corrected Scavenger Hunt Slideshow Builder"
...
# Build ffmpeg inputs (loop each temp video for IMAGE_DURATION)
INPUTS=()
...
echo "🎬 Building slideshow with proven crossfade method..."

# Build filter_complex using the correct pattern from slideshow_builder.sh
FILTER=""
TOTAL=${#TEMP_VIDEOS[@]}
...
# Process each subsequent video with crossfade
for ((i=1;i<TOTAL;i++)); do
  prev=$((i-1)) 
  offset=$((1 + OVERLAP*i))
  # Create labeled input for this video (no geometry scaling needed for temp videos)
  FILTER+="[$i:v][s$i];"
  # Crossfade previous result with this video
