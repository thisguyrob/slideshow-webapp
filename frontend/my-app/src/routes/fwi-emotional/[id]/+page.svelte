				...
				<div class="flex justify-between items-center h-16">
					...
					{#if project.images?.length > 0 && project.audio}
						<div class="flex items-center space-x-3">
							...
							{:else}
								<button
									onclick={() => startProcessing('emotional')}
			...
			<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
				<nav class="-mb-px flex space-x-8" aria-label="Tabs">
					<button
						...
						class="py-4 px-1 border-b-2 font-medium text-sm {activeTab === 'images' ? 'border-purple-500 text-purple-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
					>
						Images & Audio
					</button>
					<button
						onclick={() => activeTab = 'slideshow'}
						class="py-4 px-1 border-b-2 font-medium text-sm {activeTab === 'slideshow' ? 'border-purple-500 text-purple-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}"
					>
						Preview Slideshow
					</button>
				</nav>
